...
def cow_stat(animal_id, start_time, milkTime, action, milk, kickOff, animal_id2, off_id, off_id2, lact):
    ...
    for i in outData['Animal_ID']:
        ...
        outData.loc[outData.Animal_ID == i, "avg_Milk_duration"] = round(sum(times)/len(times),2)
        outData.loc[outData.Animal_ID == i, "total_Milk"] = (data[data['Animal_ID']==i])['Milk_yield'].sum()
        outData.loc[outData.Animal_ID == i, "Nr_of_kickOffs"] = (outData[outData['Animal_ID']==i])["numberOfMilkings"] -(data[data['Animal_ID']==i])['Nr_of_kickOffs'].isnull().sum()
